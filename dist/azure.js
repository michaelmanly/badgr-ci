...
async function run() {
  ...
  if (logs.setupError || !logs.logSnippet) {
    ...
    const posted2 = await upsertPrThread(token, setupComment);
    if (!posted2)
      await publishSummary(setupComment);
    return;
  }
  const context = collectContext({
    ...
    runId: buildId(),
    runUrl: collectionUrl() && project() && buildId() ? `${collectionUrl()}${project()}/_build/results?buildId=${buildId()}` : void 0,
